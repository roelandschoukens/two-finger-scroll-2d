...
{
	MemoryBlock block;
	DWORD type = WindowsRegistry::getBinaryValue("HKEY_CURRENT_USER\\"+name+"\\"+valueName, block);
	if (type == REG_DWORD)
	{
...
{
	WindowsRegistry::setValue("HKEY_CURRENT_USER\\"+name+"\\"+valueName, (juce::uint32) value);
}


static void deleteRegValue(const String &name, const String &valueName)
{
	WindowsRegistry::deleteValue("HKEY_CURRENT_USER\\"+name+"\\"+valueName);
}
...
StringArray TouchPad::enumApplications()
{
	...
	if (RegOpenKeyEx (rootKey, appKey.toUTF8(), 0, KEY_READ, &key) == ERROR_SUCCESS)
	{
